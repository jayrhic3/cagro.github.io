...
$query .= 'SELECT t1.lastname as lastname,t1.firstname as firstname,t1.word_created as dates,
t1.service_id as id,t1.created_at as dateni,t1.status as stat,t1.assistace_received as assis 
from (select t1.lastname as lastname,t1.firstname as firstname,t2.word_created as word_created,
t2.service_id as service_id,t2.created_at as created_at,t2.status as status,t2.assistance_recieved 
as assistace_received from beneficiaries as t1 inner join recent_request as t2 on
 t1.id=t2.beneficiary_id and t2.other="check" where t2.beneficiary_id="'.$id.'") as t1 ';

if(isset($_POST["search"]["value"]))
{
    $query .= 'WHERE t1.lastname LIKE "%'.$_POST["search"]["value"].'%" ';
    $query .= 'OR t1.firstname LIKE "%'.$_POST["search"]["value"].'%" ';
    $query .= 'OR t1.assistace_received LIKE "%'.$_POST["search"]["value"].'%" ';
